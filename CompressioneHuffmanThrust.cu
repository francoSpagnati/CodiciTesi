...
std::map<char, int> calculate_char_frequencies(const std::string& input, char** devCharOut) {
    ...
    *devCharOut = devChar;
    thrust::device_vector<char> dev_input(devChar, devChar + len);
    thrust::device_vector<int> dev_freq(256, 0);  // ASCII size
    thrust::for_each(thrust::device, dev_input.begin(), dev_input.end(),[freq_ptr = thrust::raw_pointer_cast(dev_freq.data())] __device__ (char c) {atomicAdd(&freq_ptr[(unsigned char)c], 1);});
    std::vector<int> host_freq(dev_freq.begin(), dev_freq.end());
    std::map<char, int> frequencyMap;
    for (int i = 0; i < 256; i++) {
        if (host_freq[i] > 0) {frequencyMap[(char)i] = host_freq[i];}
    }
    return frequencyMap;
}
...
int main(int argc, char* argv[]) {
    ...
    //std::cout << "devChar pointer: " << static_cast<void*>(devChar) << std::endl;
